...
export const feedbackCreateInputs = {
      ...
      subchecks: [
        ...
        },
        {
          ...
          checkfunction: (serviceInputs) => {
                console.log("Check consent is available + checkfunction", serviceInputs)
                if (serviceInputs.inputs.consent === false || serviceInputs.inputs.consent === undefined) {
                    return { 
        ...
        },
        {
            ...
            fieldsinerror: ["text"],
            checkfunction: (serviceInputs) => {
                console.log("Check text is available + checkfunction", serviceInputs)
                if (serviceInputs.inputs.text === "" && serviceInputs.inputs.source === "open") {
                    return { 
        ...
        },
        {
            ...
            fieldsinerror: ["tag"],
            checkfunction: (serviceInputs) => {
                console.log("Check tag is available + checkfunction", serviceInputs)
                if (serviceInputs.inputs.tag === "" && serviceInputs.inputs.source === "open") {
                    return { 
                        errors: ["feedback.error.emptytagbutopensource"],
                        proceed: false 
                    };
                } else {
                    return { proceed: true };
  ...
  apicall: async (inputs, log) => {
    ...
    try {
      return await apiFeedbackCreate(inputs, appStore.getState().authSlice.token);
    } catch (err) {
      return err;
    }    
  },
