...
export default function SignInModal() {
  ...
  function setStates(stateChanges) {
    Object.keys(stateChanges).forEach((change) => {
      switch (change) {
        ...
        default:
          console.error(
            "SignInModal.setStates unknown change ",
            change,
            stateChanges[change],
          );
      }
  ...
  const changes = {
    ...
    signin: () => {
      ...
      serviceSignInCheck(inputs).then((checkOutcome) => {
        setStates(checkOutcome.stateChanges);
        if (checkOutcome.proceed) {
          serviceSignIn(inputs).then((outcome) => {
    ...
    resetpassword: () => {
      ...
      serviceSendPasswordCheck(inputs).then((checkOutcome) => {
        setStates(checkOutcome.stateChanges);
        if (checkOutcome.proceed) {
          serviceSendPassword(inputs).then((outcome) => {
            setStates(outcome.stateChanges);
              ...
              {signInStatus === "inactivated" ? (
                ...
                >
                  ...
                  <LoadingButton
                    ...
                    disabled={
                      loadingSendActivation || sendActivationStatus === "sent"
                    }
