...
const rootFolder =
    'C:/Users/pierre.savoyat/Documents/cowhist19/src/App/components'

const fs = require('fs')
const getFileList = require('../_services/getFileList.js')
const getKeyList = require('../_services/getKeyList.js')

// Get the default localization file
const defaultLocalizationFile = require('../../i18n/i18n-EN.json')
...
try {
    shortenKeyList(fileList, UnusedKeyList).then((remainingKeys) => {
        ...
        console.log(
            '  - Total unsued keys : ' +
                Math.floor(
                    (remainingKeys.length / defaultLocalizationKeys.length) *
                        100
                ) +
                '% (' +
        ...
        console.log(
            '    (note : usused keys might be related to constructed keys)'
        )
    })
} catch (error) {
    console.log(error)
...
async function shortenKeyList(fileList, keyList) {
    ...
    return new Promise((resolve, reject) => {
        ...
        loadFileContent(fileList[0]).then((data) => {
            ...
            let remainingKeys = keyList.filter(
