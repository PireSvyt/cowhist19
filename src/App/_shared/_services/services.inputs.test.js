...
import { gameCreateInputs } from './game/game.service.inputs.js'
import { adminGetTablesByGamesInputs } from './admin/admin.services.inputs.js'

describe.skip('TEST OF INPUTS : services.inputs', () => {
    let testInputs = {
    ...
    Object.keys(testInputs).forEach((inputKey) => {
        ...
        test(inputKey + ' / Inputs meet expected format', () => {
            ...
            if (input.getcheckoutcomedispatchfunction !== undefined) {
                expect(typeof input.getcheckoutcomedispatchfunction).toBe(
                    'function'
                )
                expect(
                    typeof input.getcheckoutcomedispatchfunction(testLog)
                ).toBe('string')
            }
